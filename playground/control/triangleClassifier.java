...
*Problem Statement: Given three positive integer side lengths,
* first determine if they form a valid triangle (the sum of any two sides must strictly exceed the third). If invalid → NOT A TRIANGLE.
* If valid, classify by side equality: all three equal → EQUILATERAL; exactly two equal → ISOSCELES; all different → SCALENE.
* Additionally, check (independently) whether it's a right triangle (using whichever side is largest as the hypotenuse), and if so append  - RIGHT to the classification.
* Input Format: three ints a b c on one line
* Output Format: one line, e.g. SCALENE - RIGHT, EQUILATERAL, or NOT A TRIANGLE
